 ...
 */
class KnowledgeVaultFilterAlter implements FormAlterServiceBaseInterface, FormAlterServiceAlterInterface {
  ...
  public function hasMatch(&$form, FormStateInterface $form_state, $form_id) {
    return 'views-exposed-form-article-article' == $form['#id'];
  }
  ...
  public function formAlter(&$form, FormStateInterface $form_state) {
    ...
    foreach (Element::getVisibleChildren($form) as $name) {
      if (isset($form[$name]['#type']) && 'select' === $form[$name]['#type']) {
        $tags = [];
        $options = [];
        ...
        $form['title']['#attributes']['placeholder'] = t('Search in articles');
    ...
    $form['#attached']['library'][] = 'knowledge_vault/knowledge_vault.enable';
  }
